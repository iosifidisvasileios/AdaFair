...
def check_accuracy(model, x_train, y_train, x_test, y_test, y_train_predicted, y_test_predicted):
    ...
    return train_score, test_score, correct_answers_train, balanced_acc

def check_auc(model, x_train, y_train, x_test, y_test, y_train_predicted, y_test_predicted):
    if model is not None and y_test_predicted is not None:
        print ("Either the model (w) or the predicted labels should be None")
        raise Exception("Either the model (w) or the predicted labels should be None")
    ...
    if model is not None:
        y_test_predicted = np.sign(np.dot(x_test, model))
    output = np.array(y_test_predicted)
    output[output == -1] = 0
    np.savetxt('test_output.csv', output , fmt='%d',delimiter=',')


    return metrics.roc_auc_score(y_test, y_test_predicted)*100
...
def conversion(x_all, y_all, x_control_all, train_fold_size):

    split_point = int(round(float(x_all.shape[0]) * train_fold_size))
    x_all = x_all[:split_point]
    y_all = y_all[:split_point]
    x_control_all_out = {}

    for k in x_control_all.keys():
        x_control_all_out[k] = x_control_all[k][:split_point]

    return x_all, y_all, x_control_all
