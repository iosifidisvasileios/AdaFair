...
from Competitors import utils as ut
...
def train_model_disp_mist(x, y, x_control, loss_function, EPS, cons_params=None):
    ...
    if cons_params is not None:
        ...
        if take_initial_sol == True: # get the initial solution
            ...
            p.solve()
            # p.solve(solver=ECOS,max_iters=max_iters, verbose=True)
    ...
    # except:
    #     try:
    #         prob.solve(method='dccp', tau=tau + 0.5*tau, mu=mu, tau_max=1e10,
    #                    solver=ECOS, verbose=False,
    #                    feastol=EPS, abstol=EPS, reltol=EPS, feastol_inacc=EPS, abstol_inacc=EPS, reltol_inacc=EPS,
    #                    max_iters=max_iters, max_iter=max_iter_dccp)
    #     except:
...
def getRRBRate(w, x_train, y_train, x_control_train, x_test, y_test, x_control_test, sensitive_attrs):
    ...
    correct_answers = (all_class_labels_assigned_test == y_test).astype(int)  # will have 1 when the prediction and the actual label match
...
def get_clf_stats(w, x_train, y_train, x_control_train, x_test, y_test, x_control_test, sensitive_attrs):
    ...
    rates = get_tpr_dif(y_test, all_class_labels_assigned_test, x_control_test, sensitive_attrs, verbose=False)
...
def get_tpr_dif(y_true, y_pred, x_control, sensitive_attrs, verbose = True):
    # we will make some changes to x_control in this function, so make a copy in order to preserve the origianl referenced object
    x_control_internal = deepcopy(x_control)
    ...
    for s in sensitive_attrs:
        ...
        for s_val in sorted(list(set(s_attr_vals))):
            ...
            if s_val == 0:
                results['TPR_Protected'] = tpr
                results['TNR_Protected'] = tnr
            else:
                results['TPR_Non_Protected'] = tpr
                results['TNR_Non_Protected'] = tnr

            # if verbose == True:
            #     if isinstance(s_val, float): # print the int value of the sensitive attr val
            #         s_val = int(s_val)
            # print "||  %s  || %0.2f || %0.2f || %d || % d" % (s_val, tnr*100, fpr*100, tn , fp)
            # print "||  %s  || %0.2f || %0.2f || %d || %d" % (s_val, tpr*100, fnr*100, tp, fn)
