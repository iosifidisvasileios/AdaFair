...
def run_eval(dataset):
    ...
    plt.savefig("Images/cdf_" +dataset  + "_eqop.png")
...
def train_classifier(X_train, y_train, sa_index, p_Group, mode, base_learners):
    if mode == 0:
        ...
        classifier = AdaFairEQOP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, CSB="CSB1", debug=True, c=1)
    elif mode == 2:
        classifier = AdaFairEQOP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, CSB="CSB2", debug=True, c=1)
