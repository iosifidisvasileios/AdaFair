...
from my_useful_functions import calculate_performance, plot_results_of_c_impact, calculate_performance_SP, \
    plot_results_of_c_impact_SP, calculate_performanceEQOP, plot_results_of_c_impact_EQOP
...
def create_temp_files(dataset, suffixes,steps):
    for suffix in suffixes:
        outfile = open(dataset + suffix+ "_eqop", 'wb')
...
def delete_temp_files(dataset, suffixes):
    for suffix in suffixes:
        os.remove(dataset + suffix+ "_eqop")
...
def run_eval(dataset, iterations):
    ...
    results = []
    for suffix in suffixes:
        infile = open(dataset + suffix+ "_eqop", 'rb')
        temp_buffer = pickle.load(infile)
        results.append(temp_buffer.performance)
        infile.close()
    plot_results_of_c_impact_EQOP(results[0], results[1], steps, "Images/Impact_c/", dataset)
...
def train_classifier(X_train, X_test, y_train, y_test, sa_index, p_Group, dataset, mutex, mode, base_learners, c):
    if mode == 1:
        ...
        classifier = AdaFairEQOP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, CSB="CSB2", c=c)
    ...
    mutex.acquire()
    infile = open(dataset+ "_eqop", 'rb')
    dict_to_ram = pickle.load(infile)
    infile.close()
    dict_to_ram.performance[c].append(calculate_performanceEQOP(X_test, y_test, y_pred_labels, sa_index, p_Group))
    outfile = open(dataset+ "_eqop", 'wb')
