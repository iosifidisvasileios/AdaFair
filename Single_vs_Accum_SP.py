...
from my_useful_functions import calculate_performance_SP, plot_my_results_single_vs_amort_sp
...
def create_temp_files(dataset, suffixes):
    for suffix in suffixes:
        outfile = open(dataset + suffix+ "_sp_acc", 'wb')
...
def delete_temp_files(dataset, suffixes):
    for suffix in suffixes:
        os.remove(dataset + suffix+ "_sp_acc")
...
def run_eval(dataset, iterations):
    ...
    results = []
    for suffix in suffixes:
        infile = open(dataset + suffix+ "_sp_acc", 'rb')
        temp_buffer = pickle.load(infile)
        results.append(temp_buffer.performance)
        infile.close()

    plot_my_results_single_vs_amort_sp(results, suffixes, "Images/vsNoCumul/" + dataset + "_sp_single", dataset)
...
def train_classifier(X_train, X_test, y_train, y_test, sa_index, p_Group, dataset, mutex, mode, base_learners):
    ...
    if mode == 0:
        classifier = AdaFairSP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, cumul=False, CSB="CSB1")
    elif mode == 1:
        classifier = AdaFairSP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, CSB="CSB1")
