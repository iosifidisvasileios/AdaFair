...
from my_useful_functions import plot_costs_per_round_eqop, plot_costs_per_round_sp
...
def run_eval(dataset):
    ...
    plot_costs_per_round_sp("Images/Costs/" + dataset, no_cumul, cumul)


def train_classifier(X_train, y_train, sa_index, p_Group, mode, base_learners):
    if mode == 0:
        classifier = AdaFairSP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, CSB='CSB1',cumul=False)
    elif mode == 1:
        classifier = AdaFairSP(n_estimators=base_learners, saIndex=sa_index, saValue=p_Group, CSB='CSB1',cumul=True)
